...
``` 
login to UCSF VPN
ssh facbastp0015.fac.ucsf.edu # login to bastion node
ssh C15PU01.fac.ucsf.edu # login to gpu server
```
I recommend setting up an `.ssh/config`

Note, we have four CPH nodes that are reserved for Cornerstone coursework.  You can find the class GPU allocation spreadsheet [here](https://docs.google.com/spreadsheets/d/1MXRn2XNTmmn6W0mpxCQTf4BouRHDUPKnfAURVQKx9PA/edit?gid=0#gid=0).
...
VSCode and Cursor are both great tools. AI use is not encouraged for the core code parts of this project, but it's encouraged for all analysis. 

## Setting Development Environment

Starter project code is available in this github. You can clone this repository with the following command: 
```
 git clone git@github.com:yala/CPH200_25.git
```
